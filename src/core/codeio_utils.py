...
# Precompiled regex for extracting exception info from tracebacks
...
def _replace_python_tokens_outside_strings(s: str) -> str:
    """
    Replace bare Python literals (None/True/False) only outside quoted strings.
    """
    if not s:
        return s
...
def _repair_json_like(s: str) -> str:
    ...
    # Python booleans/None → JSON (only outside quoted strings).
...
def _parse_candidate_best_effort(candidate: str):
    obj = _json_load_best_effort(candidate)
    if obj is not None:
        return obj
    unescaped = _maybe_unescape_entire_string(candidate)
    if unescaped != candidate:
        return _json_load_best_effort(unescaped)
    return None
...
def _normalize_output_key(obj):
    if not isinstance(obj, dict) or "output" in obj:
        return obj
...
def _all_balanced_spans_quote_aware(s: str):
    """
    Find all balanced top-level {...} or [...] while **ignoring braces inside strings**.
    Returns a list of (start, end). end is exclusive.
    """
    stack = []
    spans = []
    top_start = None
...
def sub_extract_last_complete_json(s: str):
    ...
    if block:
        ...
        obj = _parse_candidate_best_effort(block)
        if obj is not None:
            return _normalize_output_key(obj)
        # fall through if fenced content didn’t parse

    # Prefer the last [ANSWER] block content (supports scalar answers too).
    blocks = _ANSWER_BLOCK_RE.findall(s)
    if blocks:
        ans = _collapse_double_braces(blocks[-1].strip())
        obj = _parse_candidate_best_effort(ans)
        if obj is not None:
            return _normalize_output_key(obj)

    # Scan whole text for balanced top-level {...} or [...] and try from right to left.
    spans = _all_balanced_spans_quote_aware(s)
    for start, end in reversed(spans):
        snippet = _collapse_double_braces(s[start:end]).strip()
        if not snippet:
            continue

        obj = _parse_candidate_best_effort(snippet)
        if obj is not None:
            return _normalize_output_key(obj)
...
def get_output(mainbody, funcname, args, debug=False):
    uid = shortuuid.uuid() if shortuuid is not None else uuid.uuid4().hex
