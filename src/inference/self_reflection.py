...
from core.utils import hub_root_from_cache_dir, resolve_local_snapshot_dir, format_messages

# ─── Robust eval: reuse the same extractor & comparators as check_io_pred_acc_mp ─
# This ensures eval_with_gt matches the verification step exactly, so that items
# already marked correct in the verified JSONL are never incorrectly re-reflected.
try:
    from eval.check_io_pred_acc_mp import (
        extract_last_complete_json as _robust_extract,
        _ae_equal as _robust_ae_equal,
        deep_equal as _robust_deep_equal,
    )
    _ROBUST_EVAL = True
except Exception:
    _ROBUST_EVAL = False
...
def _clamp_tail(s: str, max_chars: int) -> str:
    ...
    tail_keep = max(0, max_chars - head_keep - len(separator))
    return s[:head_keep] + separator + s[-tail_keep:] if tail_keep > 0 else s[:max_chars]
...
def _parse_gt_raw(gt):
    """gt is stored as json.dumps(actual_value) in res.actual; recover the Python value."""
    if gt is None:
        return None
...
def _compare_values(pred, gt_parsed, *, algo, is_input_task):
    """Apply the same comparison logic as check_io_pred_acc_mp.check_pred."""
    if _ROBUST_EVAL:
        if not is_input_task and algo == "ae":
            return _robust_ae_equal(pred, gt_parsed, float_tol=1e-3)
        return _robust_deep_equal(pred, gt_parsed, float_tol=1e-3)
    # Fallback when robust import failed
    try:
        gnum = float(gt_parsed) if gt_parsed is not None else None
    except Exception:
        gnum = None
    if isinstance(pred, (int, float)) and gnum is not None:
        return math.isclose(float(pred), gnum, rel_tol=1e-3, abs_tol=1e-3)
    gts = _maybe_unquote_json_string(str(gt_parsed) if gt_parsed is not None else "")
    if isinstance(pred, str) and isinstance(gts, str):
        return _norm_text(pred) == _norm_text(gts)
    return _norm_text(str(pred)) == _norm_text(str(gt_parsed))

def eval_with_gt(answer, gt, *, algo=None, io_pred=None):
    """Score predicted answer against GT, matching check_io_pred_acc_mp.py logic exactly."""
    gt_parsed = _parse_gt_raw(gt)
    is_input_task = isinstance(io_pred, str) and io_pred.startswith("input_")

    # Extract predicted value — prefer the robust multi-pass extractor
    pred = None
    if _ROBUST_EVAL:
        out_obj = _robust_extract(answer if isinstance(answer, str) else str(answer))
        if isinstance(out_obj, dict) and "output" in out_obj:
            pred = out_obj["output"]
    if pred is None:
        _, pred, _ = extract_structured_answer(answer)

    # AE input: accept "XEOF" as "X" — matches check_io_pred_acc_mp.py semantic fallback
    if (pred is not None and is_input_task and algo == "ae"
            and isinstance(pred, str) and pred.endswith("EOF")):
        stripped = pred[:-3]
        if _compare_values(stripped, gt_parsed, algo=algo, is_input_task=is_input_task):
            return {"em": 1}

    if pred is not None:
        return {"em": int(_compare_values(pred, gt_parsed, algo=algo, is_input_task=is_input_task))}

    # pred still None — fall back to raw string comparison
    fallback_gt = gt_parsed if gt_parsed is not None else gt
    return {"em": int(_norm_text(str(answer)) == _norm_text(str(fallback_gt)))}
...
def main():
    ...
    if csv_path:
        ...
        # Fresh run: always overwrite so re-runs don't accumulate duplicate rows.
        # Partial runs (with --offset / --limit) append to an existing file or create new.
        if offset == 0 and limit is None:
            with open(csv_path, "w", encoding="utf-8") as fcsv:
                fcsv.write("id,index,algo,model,temperature,em,resolved_round,success,actual,predicted,io_pred_raw\n")
    ...
    for js in load_jsonl(args.input):
        ...
        if has_gt:
            try:
                scores = eval_with_gt(init_json or draft, gt, algo=args.algo, io_pred=js.get("io_pred"))
            except Exception:
                scores = {"em": None}
        if init_json is None:
            ...
            if init_json:
                current = init_json
                if has_gt:
                    scores = eval_with_gt(current, gt, algo=args.algo, io_pred=js.get("io_pred"))
            else:
                init_json = draft
                current = init_json
        else:
        ...
        for r in range(args.reflection_rounds):
            ...
            if pass_gt_to_reviser:
                rev_user = REVISION_USER_LEAK.format(conversation=conv_text, draft=current, gt=gt, feedback=critique)
                rev_sys = REVISION_SYSTEM_LEAK
            else:
                rev_user = REVISION_USER_NOLEAK.format(conversation=conv_text, draft=current, feedback=critique)
            ...
            if has_gt:
                scores = eval_with_gt(clean_json or current, gt, algo=args.algo, io_pred=js.get("io_pred"))
        ...
        # Select the best candidate across rounds (non-regression):
        # - maximize EM when available
        # - tie-break by earlier round to preserve a good round-0 answer
        best_ro = None
        best_key = (-1, float("inf"))  # (em, round) with em higher better, round lower better
        if has_gt and round_outputs:
            for ro in round_outputs:
                s = ro.get("scores") or {}
                em = s.get("em")
                em_val = int(em) if em in (0, 1) else -1
                rnd = ro.get("round", 10**9)
                key = (em_val, -rnd)  # compare em first, then prefer smaller round via larger -round
                if key > best_key:
                    best_key = key
                    best_ro = ro
            if best_ro is not None:
                chosen = best_ro.get("output")
                if chosen is not None:
                    current = chosen
                best_scores = best_ro.get("scores")
        ...
        if has_gt and scores.get("em") == 0:
            ...
            elif args.on_mismatch == "annotate":
                status_msg = args.mismatch_message  # annotate reasoning only, keep answer
            elif args.on_mismatch == "keep":
                pass  # keep answer silently, no annotation
        ...
        # Write lesson to lessons_db if applicable
        if has_gt and critiques:
            lesson_critique = None
            if resolved_round_idx is not None and resolved_round_idx > 0:
                # Save the critique that led to resolution
                for c in critiques:
                    if c["round"] == resolved_round_idx:
                        lesson_critique = c.get("critique", "")
                        break
            elif resolved_round_idx is None:
                # Save last critique as a cautionary lesson
                lesson_critique = critiques[-1].get("critique", "")
            if lesson_critique and lesson_critique.strip():
                lesson = {
                    "ts": dt.datetime.now().timestamp(),
                    "scope": scope,
                    "feedback": lesson_critique.strip(),
                    "resolved": resolved_round_idx is not None,
                    "algo": args.algo,
                }
                with open(args.lessons_db, "a", encoding="utf-8") as f:
                    f.write(json.dumps(lesson, ensure_ascii=False) + "\n")

        entry = js.copy()
        entry.update({
            "algo": args.algo,
            "output": (final_out if (not has_gt or args.on_mismatch != "message" or scores.get("em") == 1)
        ...
        if csv_path:
            ...
            predicted = None
            try:
                # Use robust extractor so list/tuple outputs (LZW, Huffman, RLE) are
                # not silently dropped as null.  Fall back to the scalar-only extractor.
                if _ROBUST_EVAL:
                    _pobj = _robust_extract(final_out if isinstance(final_out, str) else str(final_out))
                    predicted = _pobj.get("output") if isinstance(_pobj, dict) else None
                if predicted is None:
                    _, predicted, _ = extract_structured_answer(final_out)
            except Exception:
                predicted = final_out
